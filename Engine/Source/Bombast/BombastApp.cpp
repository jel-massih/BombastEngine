...
bool BombastApp::InitInstance(HINSTANCE hInstance, HWND hWnd, int screenWidth, int screenheight, int screenX, int screenY)
{
	...
	extern IResourceLoader* CreateMaterialResourceLoader();
	extern IResourceLoader* CreateShaderResourceLoader();
...
bool BombastApp::Render()
{
	if (m_pGame)
	{
		m_pGame->VOnRender(m_pTimer->GetFrameTime(), m_pTimer->GetTime());
	}
...
void BombastApp::UpdateGame(float deltaMs, double elapsedMs)
{
	...
	if (g_pApp->m_pGame)
	{
		...
		g_pApp->m_pGame->VOnUpdate(deltaMs, elapsedMs);
	}
}